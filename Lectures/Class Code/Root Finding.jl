...
plot(getshootingpath(para,c̄+0.1,k̄).k,label="High Initial C",legend=true)
plot!(getshootingpath(para,c̄-0.1,k̄).k,label="Low Initial C",xlabel="Time",ylabel="Capital Stock")     
k0 = 4.
fun =c -> getshootingpath(para,c,k0).k[end]-k̄
...
c̄,k̄ = compute_steadystate(para)
k0=0.9*k̄
kguess = Vector(LinRange(k0,k̄,T))
error = capitalpath_residual(para,k0,kguess)
fun = kpath->capitalpath_residual(para,k0,kpath)
result = nlsolve(fun,kguess)
...
plot(solveMITshock(para,k0,20).k,label="Multivariate Solution 20",xlabel="Time",ylabel="Capital Stock")
...
sim = simulate_economy(model,(25,0.057),200)
using StatsPlots
@df sim plot(:t,:u,layout=(2,2),subplot=1,ylabel="Unemployment")
@df sim plot!(:t,:v,subplot=2,ylabel="Vacancy Rate")
@df sim plot!(:t,:w,subplot=3,ylabel="Wage")
@df sim plot!(:t,:θ,subplot=4,ylabel="Market Tightness")
...
@df df_irf plot!(:t,:z,subplot=4,ylabel="Productivity")