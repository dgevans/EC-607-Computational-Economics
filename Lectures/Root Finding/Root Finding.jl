...
plot(getshootingpath(para,c̄+0.00001,k̄).k,label="High Initial C",legend=true)
plot!(getshootingpath(para,c̄-0.00001,k̄).k,label="Low Initial C",xlabel="Time",ylabel="Capital Stock")
plot(getshootingpath(para,c̄-0.00001,k̄,500).c,label="Low Initial C",legend=true)
...
using QuantEcon,Parameters,DataFrames,Random,StatsPlots
"""
...
function simulate_economy(dmp,initial_state,T)
    ...
    return DataFrame(t=1:T,u=u[1:T],v=v,y=y,θ=θ,z=z,w=w)
end

df_sim = simulate_economy(model,(25,0.057),200)
@df df_sim plot(:t,:u,layout=(2,2),subplot=1,ylabel="Unemployment")
@df df_sim plot!(:t,:v,subplot=2,ylabel="Vacancy Rate")
@df df_sim plot!(:t,:w,subplot=3,ylabel="Wage")
@df df_sim plot!(:t,:θ,subplot=4,ylabel="Market Tightness")

"""
    impulseResponse(dmp,u0,s0,s0′,T,N=100)

Construct an impulse responce function for T periods with initial unemployment 
u0 comparing state s0 to s0′.  N =100 is the default number of simulations
to average over.
"""
function impulseResponse(dmp,u0,s0,s0′,T,N=100)
    ...
    #return difference
    df_diff = df_irf′ .- df_irf
    df_diff.t = 1:T
    return df_diff
end

#impuse response to a decrease in productivity
df_irf = impulseResponse(model,0.057,25,20,  20)

@df df_irf plot(:t,:u,layout=(2,2),subplot=1,ylabel="Unemployment")
@df df_irf plot!(:t,:v,subplot=2,ylabel="Vacancy Rate")
@df df_irf plot!(:t,:w,subplot=3,ylabel="Wage")
@df df_irf plot!(:t,:θ,subplot=4,ylabel="Market Tightness")