...
println("I have $(Threads.nthreads()) active")
println("This is my id $(Threads.threadid())")#Master Thread id is 1
...
#Now don't create as many temporary arrays...
"""
...
using BasisMatrices,Optim,ThreadTools,LinearAlgebra
...
@elapsed solvebellman(para,getV0(para));
...
@elapsed solvebellman_parallel(para,getV0(para)); 
...
@mpi_do manager begin #execute MPI code
    using MPI
...
@elapsed @mpi_do manager begin 
    ar = AR1(1.,.99,0.01)
    sample = draw_endofT_sample(ar,25000,1000) #note each process draws its own sample 
    #How to get them all together
end

@mpi_do manager begin
    samples = MPI.Gather(sample,0,comm) #Only is on one of the processes
